...
import { Column, pesananData, userData } from "@/types";
...
export default function PesananKhusus() {
    ...
    const getPesanan = async (pageNumber = 1) => {
        try {
            ...
            const res = await apiRequest({
                endpoint: `/pesanan?pengirimanId=null&filterHarga=hargaCustom&page=${pageNumber}&limit=10&search=${search}`
            });
    ...
    const handleEdit = (row: any) => {
        ...
        setForm({
            id: row.id,
            userId: row.userId,
            pengirimanId: row.pengirimanId || null,
            noSpb: row.noSpb || "",
            koli: row.koli || "",
            berat: row.berat || "",
    ...
    const handleUpdate = async () => {
        try {
            ...
            setAlert({
                type: "success",
                message: "Pesanan Khusus berhasil diupdate",
            });
            ...
            setAlert({
                type: "error",
                message: "Gagal membuat update data pesanan Khusus"
            });
            console.error("console.error", error);
        }
    }
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            setAlert({
                type: "success",
                message: "Pesanan Khusus berhasil dihapus",
            });
            getPesanan();
        } catch (error) {
            setAlert({
                type: "error",
                message: "Gagal menghapus Pesanan Khusus",
            });
